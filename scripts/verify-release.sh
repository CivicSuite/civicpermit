...
find_python() {
  local candidates=()
  if [[ -n "${CIVICPERMIT_RELEASE_PYTHON:-}" ]]; then
    candidates+=("${CIVICPERMIT_RELEASE_PYTHON}")
  fi
...
exec((root / "civicpermit" / "__init__.py").read_text(encoding="utf-8"), namespace)
...
dist = Path("dist")
wheel = dist / "civicpermit-0.2.0-py3-none-any.whl"
sdist = dist / "civicpermit-0.2.0.tar.gz"
