...
Current state: v1.0.0 public-use module release recovered through current release gates. The module includes deterministic sample checks, optional database-backed requirement and intake records, staff review queue workflows, review-required CivicZone/CivicCode context packet support, adversarial local integration mocks, CivicCore v1.1.0 release-wheel alignment, staff-only persisted intake create/read routes, and a public sample UI at `/civicpermit`. See `docs/release-recovery-status.md` for the local release gate, browser QA, and CI evidence.
