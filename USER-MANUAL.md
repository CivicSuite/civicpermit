...
Current state: v0.2.1 corrective demotion state. The module includes deterministic sample checks, optional database-backed requirement and intake records, staff review queue workflows, review-required CivicZone/CivicCode context packet support, adversarial local integration mocks, CivicCore v1.1.0 release-wheel alignment, staff-only persisted intake create/read routes, and a public sample UI at `/civicpermit`. The previous v1.0.0 release was published in error and is superseded by this honest sub-1.0.0 label.
...
CivicPermit is a FastAPI Python package pinned to the published `CivicCore v1.1.0` release wheel. The current runtime exposes:
