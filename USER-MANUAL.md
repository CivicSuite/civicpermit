...
Current state: `0.1.0` permit intake foundation release. The module includes deterministic sample checks and a public sample UI at `/civicpermit`. It does not provide legal advice, permit approvals, official completeness determinations, fee calculations, inspections, live GIS, live LLM calls, permit application ingestion, permitting-system integrations, or final staff approval.

## For IT And Technical Staff

CivicPermit is a FastAPI Python package pinned to `civiccore==0.2.0`. The current runtime exposes:
...
flowchart LR
  ...
  CivicPermit --> CivicCore["CivicCore v0.2.0"]
  CivicPermit -. released-context .-> CivicZone["CivicZone v0.1.0"]
  CivicPermit -. released-context .-> CivicCode["CivicCode v0.1.0"]
```

CivicPermit depends on CivicCore. CivicCore does not depend on CivicPermit. CivicPermit v0.1.0 uses deterministic sample requirement data only; local permit-type configuration and cross-module APIs are future work.
